...
class time_int:
    def __init__(self, grid_man, mat_man, time_opts):
        # order 
        ...
        # Time stepping
        self.max_CFL = time_opts['Max CFL']
        self.max_dt = time_opts['dt']
        ...
        self.dt = self.compute_time_step(mat_man)
        if self.dt > 0.0:
            self.idt = 1.0 / self.dt

        msg = "Timestep constrained by:"
        if self.max_dt != np.inf:
            msg += "\n\tdt  <= {:0.2e}".format(self.max_dt)
        if self.max_CFL != np.inf:
            msg += "\n\tCFL <= {:0.2e}".format(self.max_CFL)
        print(msg)
    ...
    def post_solve(self, T_sol, mat_man):
        # Update time step
    ...
    def compute_time_step(self, mat_man):
        n_tot = self.dx_arr_squared.shape[0]
        nodal_max_dt = np.zeros(n_tot)
        for i in range(n_tot):
            mat_name = self.mat_nodes[i]
            nodal_max_dt[i] = self.dx_arr_squared[i]/mat_man.get_material(mat_name).alpha
        nodal_max_dt *= 0.5*self.max_CFL

        return min(nodal_max_dt.min(), self.max_dt) 


    def check_cfl(self, mat_man):
        # don't check CFL if max CFL is set
        if self.max_CFL != np.inf:
            return None
        ...
        for i in range(n_tot):
            mat_name = self.mat_nodes[i]
            nodal_cfl[i] = 0.5*mat_man.get_material(mat_name).alpha*self.dt/self.dx_arr_squared[i]
